import { crearTransaccion, getTransacciones } from './queries'
import { crearFactura, getUltimaFactura } from './facturas-queries'
...
function parsearTigo(body: string): { monto: number; vencimiento: string; mes: string } | null {
  ...
  return {
    monto,
    vencimiento: `${año}-${mesNum}-${dia}`,
    mes: `${fechaMatch[2]} ${año}`,
  }
}
...
export async function runGmailSync(): Promise<{ nuevas_yappy: number; nuevas_tigo: number; revisadas: number }> {
  ...
  return { nuevas_yappy: nuevasYappy.length, nuevas_tigo: nuevasTigo, revisadas: yappyMsgs.length }
}
